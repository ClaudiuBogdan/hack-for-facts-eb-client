# Experimental Map Value Filters: Implementation Logic and Decisions

This document captures the full filter implementation for `/experimental/map`, including the decisions we made, why we made them, and the exact runtime semantics currently shipped.
...
- Filters are scoped only to `/experimental/map` and do not affect `/map` or chart pages.
...
Filter evaluation happens in `useExperimentalMapSeriesData` after all series vectors are prepared:
...
5. Apply value filter masking with `applyExperimentalMapValueFilters(...)`.
...
Schema lives in `src/schemas/experimental-map.ts` under `valueFilters.rules`.
...
5. Filters are scoped to experimental map route only.
