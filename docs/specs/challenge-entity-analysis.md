...
- The active entity comes from the route CUI (`/buget/$cui/...`), so no separate UAT selection step is required
...
```mdx
<QuickLinks links={[
  { label: "Your City's Budget", href: "/buget/challenges/my-city" }
]} />
```
...
New route file:
```
src/routes/buget/challenges/my-city.lazy.tsx
```
...
- [ ] Create route `/buget/challenges/my-city`
