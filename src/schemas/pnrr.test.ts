...
describe("pnrr search schema", () => {
  ...
  it("keeps only the established dashboard views", () => {
    expect(
      ["overview", "projects", "anomalies", "map", "beneficiaries"].map(
        (view) => parsePnrrSearch({ view }).view ?? "overview",
      ),
    ).toEqual(["overview", "projects", "anomalies", "map", "beneficiaries"]);
    expect(() => parsePnrrSearch({ view: "advanced" })).toThrow();
  });
  ...
  it("removes invalid panel state without the required identifier", () => {
    expect(
      parsePnrrSearchString("?panel=beneficiary&panelBeneficiaryName=Name"),
    ).toEqual({});
  });

  it("keeps national as a detailed beneficiary type filter", () => {
    expect(
      parsePnrrSearchString("?beneficiaryTypes=%5B%22national%22%5D"),
    ).toEqual({
  ...
  it("drops unsupported entity types from canonical URL state", () => {
    ...
    expect(
      parsePnrrSearchString(
        "?entityTypes=%5B%22private%22%2C%22national%22%5D",
      ),
    ).toEqual({
      entityTypes: ["private"],
    });
