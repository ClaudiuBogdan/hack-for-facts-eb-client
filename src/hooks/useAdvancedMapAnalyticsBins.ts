...
} from '@/components/maps/advanced-map-analytics/advanced-map-analytics-bins-presets-utils';
...
interface UseAdvancedMapAnalyticsBinsArgs {
...
interface UseAdvancedMapAnalyticsBinsResult {
  binsEditorState: BinsEditorState | null;
  activeBinsPreset: AdvancedMapAnalyticsBinsPreset | undefined;
  modalBinsPreset: AdvancedMapAnalyticsBinsPreset | undefined;
...
export function useAdvancedMapAnalyticsBins({
  mapState,
  updateState,
  activeSeries,
  activeSeriesId,
  activeValues,
  seriesWarnings,
}: Readonly<UseAdvancedMapAnalyticsBinsArgs>): UseAdvancedMapAnalyticsBinsResult {
  ...
  const addBinsPreset = useCallback(() => {
    ...
    const nextPreset = createDefaultAdvancedMapAnalyticsBinsPreset(presetLabel);
    nextPreset.id = createUniqueAdvancedMapAnalyticsId(mapState.binsPresets.map((preset) => preset.id));
  ...
  const regenerateBinsPreset = useCallback(
    (presetId: string): boolean => {
      ...
      const nextPreset: AdvancedMapAnalyticsBinsPreset = {
        ...preset,
        config: {
          ...preset.config,
          bins: generatedBins,
