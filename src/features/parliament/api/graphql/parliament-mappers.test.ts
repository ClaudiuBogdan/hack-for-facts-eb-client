...
describe("mapMember", () => {
  ...
  it("omits contact when profileUrl is absent or not an http(s) URL", () => {
    ...
    expect(
      mapMember({ ...abrudean, profileUrl: "not-a-url" }).contact,
    ).toBeUndefined();
  });
...
describe("mapVoteDetail (golden anchor)", () => {
  it("maps tally 275/277 and the ballot/breakdown shape", () => {
    ...
    expect(detail.groupBreakdown[0]).toMatchObject({
      groupId: "pnl-camera_deputatilor",
      pentru: 100,
    });
  ...
  it("carries only a valid official vote source URL", () => {
    expect(
      mapVoteDetail({
        ...goldenVoteDetail,
        sourceUrl: "https://www.cdep.ro/pls/steno/evot2015.nominal?idv=29892",
      }).sourceUrl,
    ).toBe("https://www.cdep.ro/pls/steno/evot2015.nominal?idv=29892");
    expect(
      mapVoteDetail({ ...goldenVoteDetail, sourceUrl: "not-a-url" }).sourceUrl,
    ).toBeUndefined();
  });
...
describe("mapVoteListItem", () => {
  ...
  it("carries the source divisionNumber when there is one", () => {
    expect(mapVoteListItem({ ...node, divisionNumber: 7 }).divisionNumber).toBe(
      7,
    );
  });

  it("leaves divisionNumber ABSENT when the source has none (never falls back to 1)", () => {
    // A division number is a checkable fact in the official record. The old
    // `?? 1` labelled every division-less vote "Divizare 1".
    expect(mapVoteListItem(node).divisionNumber).toBeUndefined();
    expect(
      mapVoteListItem({ ...node, divisionNumber: 0 }).divisionNumber,
    ).toBeUndefined();
  });
});
...
describe("mapBillSummary lastUpdatedAt (list path = lastEventDate)", () => {
  ...
  it("uses the server lastEventDate for lastUpdatedAt (no Jan-1 fallback)", () => {
    ...
    expect(mapBillSummary(base).lastUpdatedAt).toBe(
...
describe("mapBillDetail (golden anchor)", () => {
  ...
  it("builds a chronological timeline in position order with milestones + vote link", () => {
    ...
    expect(detail.timeline.find((s) => s.position === 31)?.isMilestone).toBe(
      true,
    );
...
describe("mapMemberVotingHistory", () => {
  it("maps choices and outcomes", () => {
    ...
    expect(history.votes[0]).toMatchObject({
      choice: "impotriva",
      outcome: "respins",
      chamber: "senat",
    });
...
describe("mapMemberProfile", () => {
  it("maps control items → questions and carries the EXACT total", () => {
        ...
        items: [
          {
            ...
            sourceUrl: "https://www.cdep.ro/interpelari/2026/c1",
          },
        ],
      },
      declarations: [],
    });
    expect(profile.writtenQuestions[0]).toMatchObject({
      questionId: "c1",
      status: "raspuns",
      sourceUrl: "https://www.cdep.ro/interpelari/2026/c1",
    });
    // Without the total, one page of rows reads as the complete record.
    expect(profile.writtenQuestionsTotal).toBe(137);
    expect(profile.interestDeclarations).toHaveLength(0);
    // Speeches are no longer fetched by the profile query (the intervenții tab
    // has its own cursor query) — nothing to map.
    expect(profile.spokenContributions).toBeUndefined();
  });

  it("preserves the declaration fileUrl (the link IS the declaration)", () => {
    const profile = mapMemberProfile({
      mandateKey: "1:2024:1",
      fullName: "Abrudean Mircea",
      constituencyName: "CLUJ",
      legislature: "2024",
      controlItems: { total: 0, items: [] },
      declarations: [
        {
          declarationType: "interese",
          declarationDate: "2026-06-01",
          label: "Declarație de interese 2026",
          fileUrl: "https://www.cdep.ro/declaratii/2026/1.pdf",
        },
      ],
    });
    expect(profile.interestDeclarations[0]).toMatchObject({
      category: "Declarație de interese 2026",
      fileUrl: "https://www.cdep.ro/declaratii/2026/1.pdf",
    });
  });

  it("does not fabricate epoch dates when source dates are absent", () => {
    const profile = mapMemberProfile({
      mandateKey: "1:2024:1",
      fullName: "Abrudean Mircea",
      constituencyName: "CLUJ",
      ...
      declarations: [
        {
          ...
          label: null,
          fileUrl: "https://www.cdep.ro/declaratii/1.pdf",
        },
      ],
    });

    expect(profile.writtenQuestions[0]?.submittedAt).toBeUndefined();
    expect(profile.interestDeclarations[0]?.registeredAt).toBeUndefined();
  });
