...
describe('territoryHubFixture', () => {
  ...
  it('carries the states the page has to draw', () => {
    ...
    expect(hub?.tiles.find((tile) => tile.datasetCode === 'POP107D')?.sparkline.map(([period]) => period.iso_period)).toEqual(['2021', '2023', '2024'])
  })
  ...
  it('links a county fixture into the map and budget explorer on the county basis', () => {
    ...
    expect(hub?.relatedLinks.length).toBeGreaterThan(0)
