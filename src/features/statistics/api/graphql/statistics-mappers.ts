...
export function mapDatasetDetails(
  ...
  return {
    ...
    methodology_ro: node.methodology_ro ?? null,
    methodology_en: node.methodology_en ?? null,
    data_sources_ro: node.data_sources_ro ?? null,
    data_sources_en: node.data_sources_en ?? null,
    data_sources: (node.data_sources ?? []).map((source) => ({
      name: source.name,
      type: source.type ?? null,
      type_code: source.type_code ?? null,
      link_number: source.link_number ?? null,
    })),
    observations_ro: node.observations_ro ?? null,
    observations_en: node.observations_en ?? null,
    discontinued_after_ro: node.discontinued_after_ro ?? null,
    discontinued_after_en: node.discontinued_after_en ?? null,
    successor_dataset_code: node.successor_dataset_code ?? null,
    continues_from: (node.continues_from ?? []).map((link) => ({
      dataset_code: link.dataset_code,
      last_period_ro: link.last_period_ro,
      last_period_en: link.last_period_en ?? null,
    })),
    source_last_update: node.source_last_update ?? null,
