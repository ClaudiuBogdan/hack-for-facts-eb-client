...
export function fetchDatasetPageMock(
  ...
  const matches = MOCK_EXPLORER_DATASETS.filter((dataset) => {
    ...
    if (filter.contextCode && dataset.contextCode !== filter.contextCode) return false
    if (filter.rootContextCode && rootContextCodeOf(dataset) !== filter.rootContextCode) {
      return false
    }
...
/**
 * Walks the mock context tree up to the domain the dataset sits in, which is
 * what the server's `rootContextCode` filters on.
 */
function rootContextCodeOf(dataset: StatisticsDatasetSummary): string | null {
  let node = MOCK_CONTEXT_TREE.find((entry) => entry.code === dataset.contextCode)
  const seen = new Set<string>()

  while (node && node.parentCode !== null && !seen.has(node.code)) {
    seen.add(node.code)
    const parentCode = node.parentCode
    node = MOCK_CONTEXT_TREE.find((entry) => entry.code === parentCode)
  }
