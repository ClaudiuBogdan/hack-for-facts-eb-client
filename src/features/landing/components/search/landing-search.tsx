...
} from '@/features/landing/components/search/search-parts'
import { useEntitySelection, useSearchResults } from '@/features/landing/hooks/use-landing-search'
      ...
      return (
        <Message>
          ...
          <span className="text-muted-foreground/55">
            <Trans>Nume sau identificator.</Trans>
          </span>
        </Message>
      )
      ...
      return (
        <Message>
          <span className="text-destructive">
            <Trans>Căutarea nu a răspuns.</Trans>
          </span>{' '}
          <span className="text-muted-foreground/55">
            <Trans>Încearcă din nou într-un moment.</Trans>
          </span>
        </Message>
      )
...
export function LandingSearch({
  ...
  return (
    <Autocomplete.Root
      ...
      itemToStringValue={(item: ListItem) => (isFilterItem(item) ? i18n._(item.label) : item.title)}
          ...
          >
            ...
            <div className="flex items-baseline justify-between gap-3 border-b bg-muted/40 px-4 py-3">
              <MonoLabel className="text-muted-foreground">
                {scope || (status.kind === 'results' ? t`Rezultate` : t`Caută`)}
              </MonoLabel>
              <MonoLabel className="text-muted-foreground/55">
                <Trans>Identificator</Trans>
              </MonoLabel>
            </div>
