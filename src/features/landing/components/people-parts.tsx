...
import { CONTRIBUTORS, OPEN_SEATS, REPO_URL, type Person, type Portrait } from '@/features/landing/lib/people'

/**
 * The parts the people band and the about page are built from: the social
 * glyph row, the portrait picture and its pending stand-in, and the
 * contributor wall.
 */
          ...
          {href === '#' ? (
            ...
            <span className={cn(SOCIAL_TARGET_CLASS, 'cursor-default opacity-40')}>
              ...
              <span className="sr-only">
                <Trans>{SOCIAL[kind].label} — link indisponibil</Trans>
              </span>
            </span>
          ) : (
            <a
              ...
              aria-label={t`${person.name} pe ${SOCIAL[kind].label}`}
  ...
  return (
    <ul className={cn('flex flex-wrap gap-1.5', className)}>
      {CONTRIBUTORS.map((contributor) => {
        const label = `${contributor.name} — ${i18n._(contributor.role)}`
        const picture =
      ...
      <li>
        <a
          ...
          target="_blank"
          rel="noreferrer noopener"
          title={t`Contribuie pe GitHub`}
          aria-label={t`Contribuie pe GitHub`}
          className={cn(
            AVATAR_LINK_CLASS,
