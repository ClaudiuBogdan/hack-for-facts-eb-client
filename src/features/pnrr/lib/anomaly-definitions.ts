...
export const ANOMALY_CONFIG: readonly AnomalyConfig[] = [
  ...
  },
  {
    ...
    detectionRule: t`Technical progress = 0% and financial progress > 0% or financial-technical difference > 50 pp, with technical progress under 90%`,
    icon: Banknote,
    severity: 'critical',
  },
  {
    ...
    shortDescription: t`Value over 10 mil. EUR and progress under 30%`,
    explanation: t`This filter highlights projects with large listed value and low reported technical progress. It is implementation pressure and requires verification in official documents.`,
    investigationTip: t`Check delayed procurements, authorizations, disputes, beneficiary capacity, and execution blockages.`,
    detectionRule: t`Listed value >= 10,000,000 EUR and reported technical progress under 30% or "in implementation"`,
    icon: AlertTriangle,
    severity: 'critical',
  },
  {
...
export const DATA_QUALITY_SIGNAL_CONFIG: readonly DataQualitySignalConfig[] = [
  ...
  },
  {
    ...
    detectionRule: t`Listed value >= 10,000,000 EUR and financial progress not published in dataset`,
    icon: FileQuestion,
    severity: 'attention',
  },
  {
    ...
    detectionRule: t`Reported technical progress = 100%, listed value >= 1,000,000 EUR and financial progress not published in dataset`,
    icon: FileWarning,
    severity: 'attention',
  },
] as const
