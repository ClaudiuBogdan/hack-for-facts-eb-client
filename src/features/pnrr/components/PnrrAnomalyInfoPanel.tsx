...
import type { AnomalyType, DataQualitySignalType, PnrrAggregates } from '@/schemas/pnrr'
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function PnrrAnomalyInfoPanel({
  ...
  const [expanded, setExpanded] = useState<string | null>(selectedSignal?.type ?? null)
      ...
      <SheetContent className="flex w-full flex-col gap-0 overflow-hidden border-l-2 border-[var(--pnrr-border)] bg-[var(--pnrr-bg)] p-0 sm:max-w-xl [&>button.absolute]:right-5 [&>button.absolute]:top-5 [&>button.absolute]:h-8 [&>button.absolute]:w-8 [&>button.absolute]:rounded-none [&>button.absolute]:bg-transparent [&>button.absolute]:opacity-100 [&>button.absolute]:ring-offset-0 [&>button.absolute]:hover:bg-[var(--pnrr-card)] [&>button.absolute]:focus:ring-[var(--pnrr-blue)]">
        <SheetHeader className="border-b-2 border-[var(--pnrr-border)] bg-[var(--pnrr-bg)] p-6 pr-14 text-left">
          ...
          <SheetTitle className="text-left text-4xl font-black leading-none tracking-tight text-[var(--pnrr-fg)]">
            <Trans>Ghidul semnalelor</Trans>
          </SheetTitle>
          <SheetDescription className="pt-2 text-left text-base font-bold leading-relaxed text-[var(--pnrr-muted)]">
            <Trans>
              Filtrele sunt calculate automat din datele raportate. Ele ajută la
              prioritizare, nu dovedesc singure o abatere.
            </Trans>
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="space-y-6 p-6">
            <section className="border-2 border-[var(--pnrr-border)] bg-[var(--pnrr-card)] p-5">
              <h3 className="text-sm font-black uppercase tracking-wide text-[var(--pnrr-fg)]">
                <Trans>Cum se interpretează filtrele</Trans>
              </h3>
              <div className="mt-4 space-y-4 text-sm font-medium leading-relaxed text-[var(--pnrr-muted)]">
                <p>
                  <Trans>
                    Riscurile majore indică proiecte unde progresul financiar,
                    progresul tehnic sau valoarea proiectului sugerează o verificare
                    prioritară. Sunt cele mai potrivite filtre pentru investigații de
                    fond.
                  </Trans>
                </p>
                <p>
                  <Trans>
                    Calitatea datelor marchează rânduri care pot distorsiona analiza:
                    duplicate cu informații diferite sau proiecte fără progres
                    financiar publicat.
                  </Trans>
                </p>
                <p>
                  <Trans>
                    Dacă selectați mai multe filtre, tabelul afișează proiectele care
                    îndeplinesc cel puțin unul dintre ele. Valorile sunt calculate în
                    contextul filtrelor active din pagină.
                  </Trans>
                </p>
              </div>
            </section>

            <GuideSection title={<Trans>Riscuri majore</Trans>}>
              {ANOMALY_CONFIG.map((cfg) => (
                <SignalGuideItem
                  ...
                  onToggle={() => setExpanded(expanded === cfg.type ? null : cfg.type)}
                />
              ))}
            </GuideSection>

            <GuideSection title={<Trans>Calitatea datelor</Trans>}>
            ...
            <section className="border-2 border-[var(--pnrr-border)] bg-[var(--pnrr-card)] p-5">
              <h3 className="text-sm font-black uppercase tracking-wide text-[var(--pnrr-fg)]">
                <Trans>Notă privind datele</Trans>
              </h3>
              <div className="mt-3 space-y-3 text-sm font-medium leading-relaxed text-[var(--pnrr-muted)]">
                <p>
                  <Trans>
                    Riscurile sunt separate de problemele de calitate a datelor pentru
                    a evita alarmele false și pentru a păstra clară diferența dintre
                    execuție și raportare.
                  </Trans>
                </p>
                <p>
                  <Trans>
                    {aggregates.missingFinProgressPercent.toFixed(0)}% din proiecte
                    nu au date despre progresul financiar, ceea ce poate ascunde
                    riscuri care depind de comparația dintre progresul tehnic și cel
                    financiar.
                  </Trans>
                </p>
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[auto_1fr_auto] gap-4 p-4 pl-5">
        <span className={cn('flex h-11 w-11 items-center justify-center border-2', tone.border, tone.bg, tone.text)}>
      ...
      {expanded && (
        <div className="border-t-2 border-[var(--pnrr-border)] bg-[var(--pnrr-card)] p-4 pl-5">
          <div className="space-y-4">
            <SignalDetail label={<Trans>Regulă de detectare</Trans>}>
              <span className="inline-flex items-start gap-2">
                <span className={cn('mt-1.5 h-2.5 w-2.5 shrink-0', tone.accent)} />
            ...
            <SignalDetail label={<Trans>Ce să investigați</Trans>}>
              {cfg.investigationTip}
            </SignalDetail>
            {hasCount && (
              <div className="grid grid-cols-2 gap-3 border-t border-[var(--pnrr-border)] pt-4">
                <SignalStat label={<Trans>Proiecte</Trans>} value={String(data.count)} />
                <SignalStat
                  label={<Trans>Valoare totală</Trans>}
