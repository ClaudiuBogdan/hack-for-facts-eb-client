...
export function PnrrHeader({
  ...
  const formattedLastUpdated = new Date(PNRR_LAST_UPDATED).toLocaleDateString('ro-RO', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
  })
    ...
    <>
      ...
      <div
        ...
        className={cn(
          ...
          !isMobile && sidebarState === 'expanded' && 'left-[var(--sidebar-width)] right-0 w-auto',
          !isMobile && sidebarState === 'collapsed' && 'left-[var(--sidebar-width-icon)] right-0 w-auto',
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3 overflow-hidden">
            ...
            <PnrrTabNav view={view} onChange={onViewChange} compact className="border-0" />
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 pb-6 pt-10 sm:px-6 sm:pt-14 lg:px-8">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-2">
              {isLoading ? (
                <>
                  <div className="inline-flex items-center gap-2 border-2 border-[var(--pnrr-border)] px-3 py-2 text-[var(--pnrr-fg)]" style={{ backgroundColor: 'var(--pnrr-card)' }}>
                    <Activity className="h-4 w-4 text-current" />
                    <Skeleton className="h-4 w-8 rounded-none" />
                    <span className="text-sm font-bold text-current"><Trans>proiecte</Trans></span>
                  </div>
                  <div className="inline-flex items-center gap-2 border-2 border-[var(--pnrr-border)] px-3 py-2 text-[var(--pnrr-fg)]" style={{ backgroundColor: 'var(--pnrr-card)' }}>
                    <Skeleton className="h-4 w-24 rounded-none" />
                    <span className="text-sm font-bold text-current"><Trans>total</Trans></span>
                  </div>
                </>
              ) : (
                <>
                  <div className="inline-flex items-center gap-2 border-2 border-[var(--pnrr-border)] px-3 py-2 text-[var(--pnrr-fg)]" style={{ backgroundColor: 'var(--pnrr-card)' }}>
                    <Activity className="h-4 w-4 text-current" />
                    <span className="text-sm font-black text-current">{projectsCount.toLocaleString('ro-RO')}</span>
                    <span className="text-sm font-bold text-current"><Trans>proiecte</Trans></span>
                  </div>
                  <div className="inline-flex items-center gap-2 border-2 border-[var(--pnrr-border)] px-3 py-2 text-[var(--pnrr-fg)]" style={{ backgroundColor: 'var(--pnrr-card)' }}>
                    <span className="text-sm font-black text-current">{formatPnrrCurrency(totalValue, currency)}</span>
                    <span className="text-sm font-bold text-current"><Trans>total</Trans></span>
                  </div>
                </>
              )}
