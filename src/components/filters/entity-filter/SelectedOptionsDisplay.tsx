...
import { EntityOption } from "./interfaces"; // Assuming interfaces.ts is in the same directory
...
export function SelectedOptionsDisplay({
    ...
    return (
        <div className={cn("px-4 pt-3 pb-3 border-b w-full", className)}> {/* Added pt-3 for consistency */}
            ...
            {shouldShowCompactBadgeSummary ? (
                <div className={cn("flex items-center gap-2")}>
                    {itemsToShowInCompactSummary.map((option) => (
                        ...
                        >
                            <span className="truncate max-w-[100px] block" title={option.name}>
                                {option.name}
                            </span>
                            <button
                ...
                <ScrollArea className={cn("w-full whitespace-nowrap")}>
                    <div className={cn("flex gap-2 pb-2")}>
                        {selected.map((option) => (
                            ...
                            >
                                <span className="truncate max-w-[150px] block" title={option.name}>
                                    {option.name}
                                </span>
                                <button
                                    onClick={() => toggleSelect(option)}
